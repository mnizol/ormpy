...
class NormaLoader(object):
    """ Loads an .orm file produced by the NORMA modeling tool into a
        :class:`Model.Model`.  There are no public methods in this class.  To
        load a model, just call:: 
 
            loader = NormaLoader("/path/to/file/example.orm")
            model = loader.model  

        The only other publically accessible attribute is the *omissions* list,
        which contains a list of model elements the loader ignored.      
    """
    ...
    def __init__(self, filename):
        ...
        self.model = Model() # Create a new ORMPY model
        self.omissions = []  # Omitted model elements to report to the user
